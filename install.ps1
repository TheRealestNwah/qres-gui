<#
.SYNOPSIS
Installs QRes GUI for the current user.
...
Works from a source checkout (builds first) or from an unpacked release zip
(installs the files next to it).

.PARAMETER SkipBuild
Source checkout only: install the existing dist\QResGUI build instead of rebuilding.
#>
...
if (Test-Path (Join-Path $root "qres_gui")) {
    if (-not $SkipBuild) {
        & (Join-Path $root "build.ps1")
        if ($LASTEXITCODE) { exit $LASTEXITCODE }
    }
    $source = Join-Path $root "dist\QResGUI"
    $uninstallScript = Join-Path $root "installer\uninstall.ps1"
    $version = (Select-String -Path (Join-Path $root "qres_gui\__init__.py") -Pattern '__version__ = "(.+)"').Matches[0].Groups[1].Value
}
else {
    # Unpacked release: the app files sit next to this script.
    $source = $root
    $uninstallScript = Join-Path $root "uninstall.ps1"
    $version = (Get-Content (Join-Path $root "VERSION") -Raw).Trim()
}
if (-not (Test-Path (Join-Path $source "QResLauncher.exe"))) {
    throw "No build in $source - run without -SkipBuild."
}
...
Copy-Item $uninstallScript (Join-Path $dest "uninstall.ps1") -Force
